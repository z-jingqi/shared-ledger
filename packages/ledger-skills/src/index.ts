...
export const ledgerSkillRegistry: LedgerSkillDefinition[] = [
  ...
  },
  {
    ...
    tools: [
      tool("create-record", "新增一笔或多笔收入/支出。", "{ type, amount, occurredAt?, categoryId?, categoryName?, note?, items? } 或 { records: [...] }", ["text", "record-card"], "never", ["book:write"]),
      tool("update-record", "修改已有交易，可用 transactionId 或 relative='last'。", "{ transactionId?, relative?, amount?, type?, occurredAt?, categoryId?, categoryName?, note?, items? }", ["text", "record-card"], "never", ["transaction:own-write"]),
      tool("delete-record", "删除已有交易，支持单条或批量。", "{ transactionId?, transactionIds?, relative?, amount?, note?, q? }", ["text"], "always", ["transaction:own-delete"]),
    ],
  },
  {
    name: "ledger.search",
  ...
  },
  {
    ...
    tools: [tool("analyze-records", "基于真实交易做分析。", "{ type?, minAmount?, maxAmount?, from?, to?, categoryId?, categoryName?, q?, limit? }", ["text", "analysis-card"], "never", ["book:read"])],
  },
  {
    name: "ledger.imports",
    title: "导入与识别",
    ...
    tools: [
      tool("save-attachments", "把附件提交为导入任务。", "{ autoConfirm? }", ["text", "import-job-card"], "never", ["book:write"]),
      tool("confirm-import-batch", "确认导入批次中的待确认记录。", "{ importJobId?, recordIds? }", ["text", "pending-record-card"], "always", ["book:write"]),
      tool("cancel-task", "取消导入或 AI 任务。", "{ taskId }", ["text", "import-job-card"], "always", ["book:write"]),
      tool("retry-task", "重试失败任务。", "{ taskId }", ["text", "import-job-card"], "never", ["book:write"]),
    ],
  },
  {
    ...
    useWhen: "用户要维护分类。",
    tools: [
      tool("create-category", "新增当前用户的个人分类。", "{ name, type, icon?, sortOrder? }", ["text"], "never", ["account:write"]),
      tool("update-category", "修改当前用户的个人分类。", "{ id?, name?, newName?, type?, icon?, sortOrder? }", ["text"], "never", ["account:write"]),
      tool("delete-category", "删除当前用户的个人分类。", "{ id?, name?, type? }", ["text"], "always", ["account:write"]),
    ],
  },
  {
    name: "ledger.books",
    ...
    useWhen: "用户要管理账本本身。",
    tools: [
      tool("create-book", "创建新账本。", "{ name, currency? }", ["text", "navigation-card"], "never", ["account:write"]),
  ...
  },
  {
    ...
    tools: [
      tool("invite-member", "邀请成员加入账本。", "{ target?, email?, phone?, userId?, role? }", ["text", "member-card"], "always", ["book:admin"]),
      tool("update-member", "修改成员角色。", "{ memberId?, userId?, name?, role }", ["member-card"], "never", ["book:admin"]),
      tool("remove-member", "移除成员或当前用户退出。", "{ memberId?, userId?, name?, self? }", ["text"], "always", ["book:admin"]),
    ],
  },
  {
    name: "ledger.profile",
    ...
    tools: [tool("update-profile", "修改当前用户资料。", "{ name?, email?, avatarFromAttachment? }", ["text", "profile-card"], "never", ["account:write"])],
  },
  {
    ...
    tools: [tool("chat", "回答套餐相关说明；需要真实订阅数据时使用上下文。", "{ message?: string }", ["text"], "never", ["account:read"])],
  },
  {
    name: "ledger.export",
    title: "导出",
    ...
    tools: [tool("export-book", "生成导出入口。", "{ bookId? }", ["text", "navigation-card"], "always", ["book:read"])],
  },
  {
    ...
    tools: [tool("chat", "返回导航建议或说明。", "{ message?: string }", ["text", "navigation-card"], "never", [])],
  },
];
...
export function getLedgerTool(toolName: LedgerToolName, skillName?: LedgerSkillName) {
  const skills = skillName ? ledgerSkillRegistry.filter((skill) => skill.name === skillName) : ledgerSkillRegistry;
  return skills.flatMap((skill) => skill.tools.map((toolDefinition) => ({ ...toolDefinition, skillName: skill.name }))).find((toolDefinition) => toolDefinition.name === toolName);
}
