...
const cookieOptions = (environment?: string) => ({
  httpOnly: true,
  ...
  app.post("/auth/register", async (context) => {
    ...
    try {
      if (context.env.DB) {
        ...
        setCookie(context, "ledger_session", token, cookieOptions(context.env.APP_ENV));
        return context.json({ user }, 201);
      }
      if (context.env.APP_ENV !== "test" || !store) return jsonError(context, "认证需要 D1 运行时", 503);
      return context.json({ user: store.createUser(body.name, body.email ?? "") }, 201);
    } catch (error) {
  ...
  app.post("/auth/login", async (context) => {
    ...
    if (context.env.DB) {
      ...
      setCookie(
        context,
        "ledger_session",
        await createSession(context.env.DB, user.id),
        cookieOptions(context.env.APP_ENV),
      );
      return context.json({ user });
    }
    if (context.env.APP_ENV !== "test" || !store) return jsonError(context, "认证需要 D1 运行时", 503);
    const user = store.users.find((item) => item.email === body.identifier || item.id === body.identifier);
